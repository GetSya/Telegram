...
bot.on("new_chat_members", async(lol) => {
    ...
    for (x of message.new_chat_members) {
        ...
        await lol.replyWithPhoto({ url: `https://melcanz.com/welcome?username=${full_name}&groupname=${groupname}&groupicon=${pp_group}&membercount=${groupmembers}&profile=${pp_user}&background=${pp_user}&apikey=${apikey}` })
    }
})
...
bot.on("left_chat_member", async(lol) => {
    ...
    await lol.replyWithPhoto({ url: `https://melcanz.com/goodbye?username=${full_name}&groupname=${groupname}&groupicon=${pp_group}&membercount=${groupmembers}&profile=${pp_user}&background=${pp_user}&apikey=${apikey}` })
})
...
case 'nuliskanan':
              jird = body.slice(12)
                if (args.length == 0) return await reply(`Example: ${prefix + command} Arasya`)
                reply("Tunggu Sebentar, Sedang Proses....")
                await lol.replyWithPhoto({ url: `https://hardianto.xyz/api/nuliskanan?text=${jird}&apikey=hardianto` })
                break
case 'foliokiri':
              jird = args.join(" ")
                if (args.length == 0) return await reply(`Example: ${prefix + command} Arasya`)
                reply("Tunggu Sebentar, Sedang Proses....")
                await lol.replyWithPhoto({ url: `https://hardianto.xyz/api/foliokiri?text=${jird}&apikey=hardianto` })
                break
                case 'foliokanan':
              jird = args.join(" ")
                if (args.length == 0) return await reply(`Example: ${prefix + command} Arasya`)
                reply("Tunggu Sebentar, Sedang Proses....")
                await lol.replyWithPhoto({ url: `https://hardianto.xyz/api/foliokanan?text=${jird}&apikey=hardianto` })
                break
//api amel
case 'glitch':
              jird = args.join(" ")
